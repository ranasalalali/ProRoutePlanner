...
import { addRickshawWayPoint, getRickshawWaypointList, addRickshawStartPoint, addRickshawEndPoint, getSearchPredictions, getSearchInput, getSelectedRegion } from '../../actions';
import { Actions } from 'react-native-router-flux';
import { connect } from 'react-redux';
import styles from '../AdminStyles';

class RickshawEndPointMap extends Component {
  constructor(props) {
    super(props);
    this.state = {
      place: '',
    ...
    return (
      <Container>
        <Header style={{ backgroundColor: 'black' }} androidStatusBarColor='black'>
          <Left>
            <Button transparent onPress={() => Actions.rickshawstartpointmap()}>
        ...
        <Button full dark onPress={() => this.AddRickshawEndpoint()}>
          <Icon name='add' />
          <Text>Add</Text>
        </Button>
...
const mapStateToProps = ({ auth }) => {
  const { rickshawname, rickshawerror, rickshawadded, predictions, searchinput, selectedregion } = auth;
  return { rickshawname, rickshawerror, rickshawadded, predictions, searchinput, selectedregion };
};

export default connect(mapStateToProps, { addRickshawWayPoint, getRickshawWaypointList, addRickshawEndPoint, addRickshawStartPoint, getSearchPredictions, getSelectedRegion, getSearchInput })(RickshawEndPointMap);
